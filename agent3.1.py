...
def subgraph_entry_router(state: AgentState) -> str:
    ...
    if mode == "workflow" and state.get("workflow_steps"):
        ...
        return "sub_animal_date_analysis_chat"  # 原 "chat"
...
builder.add_conditional_edges("prepare_launch", mode_router, {
    ...
    "sub_animal_date_analysis_chat": "sub_animal_date_analysis_chat",  # 原 "chat"
})
...
# 报告结果后回到子图chat（LLM 会总结结果并告知用户）
builder.add_edge("report_results", "sub_animal_date_analysis_chat")
...
# ============================================================
# 父图：负责对话交互与规划，并将任务分发至子图
# ============================================================

# ---------- 父图专用工具 ----------
@tool
def plan_skill(skill_name: str, params_json: str) -> str:
    """规划并启动技能执行（父图专用，委托子图）

    Args:
        skill_name: 技能名称或id
        params_json: 参数字符串，JSON格式，如 {"work_dir": "./data", "input_file": "input.xlsx"}
    """
    # 验证技能是否存在
    full_skill, err = _load_full_skill(skill_name)
    if full_skill is None:
        return err
    # 解析参数
    try:
        ...
        return "参数JSON格式错误"
    # 返回确认信息（由父图 plan 节点解析）
    return json.dumps({
        "action": "plan_skill",
        "skill_name": skill_name,
        "params": params,
        "skill_type": full_skill.get("type"),
        "has_workflow": bool(full_skill.get("workflow") or full_skill.get("workflow_steps"))
    }, ensure_ascii=False)


parent_tools = [plan_skill, get_skill_info, list_files, read_xlsx, read_docx]  # 不包含执行脚本的工具
parent_llm_with_tools = llm.bind_tools(parent_tools)


# ---------- 父图节点 ----------
def parent_chat_node(state: AgentState):
    """父图对话节点，负责与用户交互，决定何时调用规划工具"""
    messages = [SystemMessage(
        content="你是数据分析助手。当用户要求执行某项技能时，调用 plan_skill 工具进行规划，不要自行编造执行结果。")] + \
               state["messages"]
    response = parent_llm_with_tools.invoke(messages)
    # 打印调试信息
    if hasattr(response, 'tool_calls') and response.tool_calls:
        tc_summary = ", ".join(f"{tc['name']}({tc['args']})" for tc in response.tool_calls)
        print(f"[PARENT] LLM 调用工具 → {tc_summary}")
    elif response.content:
        print(f"[PARENT] LLM 回复 → {response.content[:100]}...")
    return {"messages": [response]}


def parent_after_tools_router(state: AgentState) -> str:
    """工具执行后的路由：检查是否有 plan_skill 调用"""
    last_msg = state["messages"][-1] if state["messages"] else None
    if isinstance(last_msg, AIMessage) and hasattr(last_msg, 'tool_calls'):
        for tc in last_msg.tool_calls:
            if tc["name"] == "plan_skill":
                # 检查对应的 ToolMessage 是否成功
                for msg in reversed(state["messages"]):
                    if isinstance(msg, ToolMessage) and msg.tool_call_id == tc["id"]:
                        content = msg.content
                        if "plan_skill" in content:
                            return "plan"
                        else:
                            # 错误信息，回到 chat
                            return "parent_chat"
...
def plan_node(state: AgentState):
    """规划节点：解析 plan_skill 工具调用，准备子图状态"""
    # 找到最近一次 plan_skill 的 ToolMessage
    plan_data = None
    for msg in reversed(state["messages"]):
        if isinstance(msg, ToolMessage) and "plan_skill" in msg.content:
            try:
                ...
                continue
    if not plan_data:
        return {"error": "未找到有效的规划数据", "execution_mode": "chat"}

    skill_name = plan_data.get("skill_name")
    params = plan_data.get("params", {})
    full_skill, err = _load_full_skill(skill_name)
    if full_skill is None:
        return {"error": err, "execution_mode": "chat"}
    ...
    return {
        ...
        "pa": params.get("pa", ""),
        "regional_level": params.get("regional_level", ""),
        "history_file": params.get("history_file", ""),
        "review_feedback": None,
        "approved": None,
        "review_action": None,
    }
...
def process_result_node(state: AgentState):
    """处理子图执行结果，生成反馈消息"""
    ...
    if status == "completed":
        msg = f"✅ 技能 '{skill_name}' 执行成功。\n"
        # 提取输出文件信息
        step_outputs = final_output.get("step_outputs", {})
        for key, val in step_outputs.items():
            if isinstance(val, dict) and val.get("type") != "review":
                files = val.get("output_files", [])
                if files:
        ...
        error_detail = final_output.get("error", "未知错误")
        msg = f"❌ 技能 '{skill_name}' 执行出错：{error_detail}"
    elif status == "aborted":
        msg = f"⚠️ 技能 '{skill_name}' 已终止。"
    else:
        msg = f"技能 '{skill_name}' 执行结束，状态: {status}"

    # 将结果消息加入对话，以便父图 chat 节点回复用户
    return {"messages": [SystemMessage(content=msg)]}
...
parent_builder.add_node("parent_tools", ToolNode(parent_tools))
parent_builder.add_node("plan", plan_node)
parent_builder.add_node("subgraph", animal_date_analysis_agent)
parent_builder.add_node("process_result", process_result_node)

# 入口 → parent_chat
parent_builder.add_edge(START, "parent_chat")
parent_builder.add_edge("parent_chat", "parent_tools")
# parent_tools → 条件路由
parent_builder.add_conditional_edges("parent_tools", parent_after_tools_router, {
    "parent_chat": "parent_chat",
    "plan": "plan"
})

# plan → subgraph
parent_builder.add_edge("plan", "subgraph")
# subgraph → process_result
parent_builder.add_edge("subgraph", "process_result")
# process_result → parent_chat（将结果反馈给对话）
parent_builder.add_edge("process_result", "parent_chat")

# 编译父图（使用同一个 checkpointer）
