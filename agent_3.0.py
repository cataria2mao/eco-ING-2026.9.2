...
# ---------- 向量检索（模拟） ----------
# 实际应使用 Chroma/FAISS 等，这里用简单的关键词匹配演示
def retrieve_docs(query: str) -> List[str]:
    """从模拟的向量库检索相关文档（技能描述或知识）"""
    # 假设有一个技能描述文档库
    doc_corpus = [
        "技能 '物种名录整理' 用于从调查数据中提取动物物种列表并生成标准名录。",
        "技能 '栖息地分析' 根据物种分布数据计算栖息地适宜性指数。",
        "技能 '种群数量估算' 采用标记重捕法或样线法估算种群大小。",
        "通用数据处理技能可读取 Excel/Word 文件，执行数据清洗和统计分析。"
        ""
    ]
    # 简单匹配（实际应使用 embedding）
    results = []
    query_lower = query.lower()
    for doc in doc_corpus:
        if any(kw in doc.lower() for kw in query_lower.split()):
            results.append(doc)
    return results if results else ["未找到相关文档，请尝试更具体的描述。"]


# ---------- 规划节点 ----------
def plan_node(state: AgentState) -> Dict:
    """根据检索结果和用户消息，规划要执行的技能及参数"""
    user_message = state["messages"][-1].content if state["messages"] else ""
    retrieved = state.get("retrieved_docs", [])
    context = "\n".join(retrieved)

    prompt = f"""根据用户需求，从可用技能中选择最合适的技能并提取参数。
可用技能列表：
{json.dumps([{"name": s["name"], "id": s["id"], "description": s["description"]} for s in skill_registry], ensure_ascii=False, indent=2)}
...
请以 JSON 格式返回规划结果，包含字段：
- "selected_skill": 技能名称或id
- "params": 参数字典（如 work_dir, input_file 等）
- "execution_mode": "workflow" 或 "single_skill"（根据技能类型）
...
"""
    ...
    skill_name = plan.get("selected_skill")
    if not skill_name:
        return {"error": "无法确定要使用的技能", "execution_mode": "chat"}
    ...
    # 确保关键字段存在
    params.setdefault("work_dir", ".")
    params.setdefault("input_file", "")
...
# ---------- 父图状态 ----------
# 复用 AgentState，额外字段已包含 (retrieved_docs, plan)
# 为了类型完整，可再定义一个 ParentState，但此处直接使用 AgentState
...
parent_builder.add_node("retrieve", retrieve_node)

# 规划节点
parent_builder.add_node("plan", plan_node)

# 子图节点（使用已编译的 animal_date_analysis_agent）
parent_builder.add_node("subgraph", animal_date_analysis_agent)

# 输出节点（提取 final_output 作为响应）
def output_node(state: AgentState) -> Dict:
    ...
    if final:
        if final.get("status") == "completed":
            ...
            msg = f"❌ 执行出错：{final.get('error', '未知错误')}"
        else:
            msg = f"任务状态：{final.get('status')}"
    else:
        msg = "子图未返回有效结果。"
    return {"messages": [SystemMessage(content=msg)]}

parent_builder.add_node("output", output_node)

# ---------- 父图边 ----------
parent_builder.add_edge(START, "retrieve")
parent_builder.add_edge("retrieve", "plan")
parent_builder.add_edge("plan", "subgraph")   # 将规划后的状态传给子图
parent_builder.add_edge("subgraph", "output")
parent_builder.add_edge("output", END)

# 编译父图（使用同一个 checkpointer，便于状态持久化）
