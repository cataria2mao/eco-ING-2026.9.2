...
def _load_full_skill(skill_name: str) -> tuple:
    ...
    Returns:
        成功: (full_skill_dict, json_path_str)
        失败: (None, error_msg_str)
    """
...
## ⚠️ 严禁事项
- **绝对不能**在未调用 launch_skill 的情况下声称技能已执行或完成
- **绝对不能**编造、模拟或推测脚本的执行结果
- 如果你还没有调用 launch_skill，用户问执行结果时，必须承认尚未执行
- launch_skill 是唯一能真正启动脚本执行的工具，没有其他方式
...
## 错误处理
当收到脚本执行错误信息时，你必须：
1. 分析错误原因（常见：文件不存在、参数格式错误、依赖缺失、编码问题、列名不匹配等）
2. 向用户清晰地说明问题
3. 提供具体的修复建议（如：检查文件路径、修改参数、安装依赖等）
4. 如果是参数问题，主动建议正确的参数值并询问是否重新执行"""
    return prompt
...
def launch_skill(skill_name: str, params_json: str) -> str:
    ...
    Args:
        skill_name: 技能名称或ID（必须与技能列表中的名称一致）
        params_json: 参数JSON字符串，键名必须与技能参数名一致。常见参数：
            work_dir（工作目录）, sample_file（样线表）, history_file（历史资料）,
            input_file（输入文件）, output_file（输出文件）, pa（居留型标记）, regional_level（省级保护级别）。
            只包含用户明确提供或确认的参数，未提供的不要传。
            示例: {"work_dir": "./data", "sample_file": "样线表.xlsx"}
    """
    # 解析参数 JSON
    try:
    ...
    return json.dumps({
        ...
        "skill_type": full_skill.get("type", "single"),
...
def build_params(state: AgentState, step: Dict, target_config: Dict) -> Dict:
    ...
    # 输出文件名推导
    skill_id = target_config.get("id", "")
    if skill_id == "animal_simple_list":
        params.setdefault("output_file", "动物列表.xlsx")
    elif skill_id == "animal_catalog_generate":
        params.setdefault("output_file", "动物名录.xlsx")
...
def chat_node(state: AgentState):
    ...
    # 构建消息列表
    messages = [SystemMessage(content=SYSTEM_PROMPT)] if not state.get("messages", []) else state["messages"] + state["messages"]
...
def prepare_launch_node(state: AgentState):
    ...
    # 合并参数到状态
    param_fields = [
        "work_dir", "sample_file", "history_file",
        "pa", "regional_level",
        "input_file", "output_file", "ref_file", "province"
    ]
...
def execute_skill(state: AgentState, step: Dict, step_idx: int) -> Dict:
    ...
    for wf_step in target_config.get("workflow", []):
        ...
        if tool_name == "run_python":
            ...
            result = run_python(wf_step["params"]["script_path"], kw_str)
            ...
            keywords = substitute_params(wf_step["params"]["keywords"], params)
            kw_str = " ".join(keywords) if isinstance(keywords, list) else keywords
            result = run_r(wf_step["params"]["script_path"], kw_str)
            is_success = not result.startswith(("错误", "执行失败"))
            results.append({
                "tool": "run_r",
                "script": wf_step["params"]["script_path"],
...
def single_executor_node(state: AgentState):
    ...
    for idx, step in enumerate(workflow):
        ...
        if tool_name == "run_python":
            result = run_python(params["script_path"], kw_str)
            is_success = not result.startswith(("错误", "执行失败"))
            results.append({
                "tool": "run_python",
            ...
            result = run_r(params["script_path"], kw_str)
            is_success = not result.startswith(("错误", "执行失败"))
            results.append({
                "tool": "run_r",
                "script": params["script_path"],
