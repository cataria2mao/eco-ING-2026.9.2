...
# 状态定义
# ------------------------------------------------------------
# 动物子图、植物子图并行执行，因此两个子图的执行状态必须字段级隔离：
#   动物子图 -> animal_* 命名空间
#   植物子图 -> plant_*  命名空间
# messages 为父图统一对话历史，子图执行路径不写 messages，故可共享。
# ============================================================


class AnimalAnalysisState(TypedDict):
    """动物数据分析子图状态（animal_date_analysis_agent）。"""
    messages: Annotated[list, add_messages]
    animal_execution_mode: str
    animal_skill_registry: Optional[List[Dict]]
    animal_selected_skill: Optional[str]
    animal_skill_config: Optional[Dict]
    animal_skill_params: Optional[Dict]
    animal_work_dir: str
    animal_input_file: str
    animal_history_file: str
    animal_pa: str
    animal_regional_level: str
    animal_current_step_idx: int
    animal_workflow_steps: List[Dict]
    animal_step_outputs: Dict[str, Any]
    animal_review_feedback: Optional[str]
    animal_approved: Optional[bool]
    animal_review_action: Optional[str]
    animal_retry_count: Dict[str, int]
    animal_retry_target_idx: Optional[int]
    animal_final_output: Optional[Any]
    animal_error: Optional[str]
    animal_domain: Optional[str]


class PlantAnalysisState(TypedDict):
    """植物数据分析子图状态（plant_date_analysis_agent）。"""
    messages: Annotated[list, add_messages]
    plant_execution_mode: str
    plant_skill_registry: Optional[List[Dict]]
    plant_selected_skill: Optional[str]
    plant_skill_config: Optional[Dict]
    plant_skill_params: Optional[Dict]
    plant_work_dir: str
    plant_input_file: str
    plant_history_file: str
    plant_regional_level: str
    plant_current_step_idx: int
    plant_workflow_steps: List[Dict]
    plant_step_outputs: Dict[str, Any]
    plant_review_feedback: Optional[str]
    plant_approved: Optional[bool]
    plant_review_action: Optional[str]
    plant_retry_count: Dict[str, int]
    plant_retry_target_idx: Optional[int]
    plant_final_output: Optional[Any]
    plant_error: Optional[str]
    plant_domain: Optional[str]


class ParentState(TypedDict):
    """父图状态：统一对话 + 两个子图的命名空间状态 + 协调层字段。"""
    messages: Annotated[list, add_messages]
...
def _log_messages(state, node_name: str = ""):
    """打印当前消息流的摘要，便于调试"""
...
def build_analysis_subgraph(
    *,
    ...
    """构建一个命名空间隔离的数据分析子图（动物或植物）。

    prefix: 状态字段前缀（"animal_" 或 "plant_"）。
    messages 保持共享（子图执行路径不写 messages），其余执行字段全部加前缀。
    """
    ...
    def build_params(state, step: Dict, target_config: Dict) -> Dict:
        params = {
            "work_dir": state.get(K("work_dir"), "."),
            "input_file": state.get(K("input_file"), ""),
            "history_file": state.get(K("history_file"), ""),
            "pa": state.get(K("pa"), ""),
            "regional_level": state.get(K("regional_level"), "")
        }
    ...
    def prepare_launch_node(state):
        ...
        state_update = {
            K("execution_mode"): mode,
            K("selected_skill"): skill_name,
            K("skill_config"): full_skill,
            K("workflow_steps"): workflow_steps,
            K("current_step_idx"): 0,
            K("step_outputs"): {},
            K("retry_count"): {},
            K("review_feedback"): None,
            K("approved"): None,
            K("review_action"): None,
            K("error"): None,
            K("final_output"): None,
            K("skill_params"): params.copy(),
            K("domain"): domain,
        }

        param_fields = ["work_dir", "input_file", "history_file", "pa", "regional_level"]
        for field in param_fields:
            if field in params and params[field]:
                state_update[K(field)] = params[field]
    ...
    def execute_review(state, step: Dict, step_idx: int) -> Dict:
        ...
        workflow_name = state[K("skill_config")].get("name", "未命名工作流")
        retry_count = state[K("retry_count")].get(f"step_{prev_output.get('step_num', 'unknown')}", 0)
        ...
        if next_steps:
            for i, ns in enumerate(next_steps, 1):
                lines.append(f"  {i}. [{ns.get('type', 'unknown')}] 步骤 {ns.get('step', 'N/A')}: {ns.get('description', '未描述')}")
        else:
            lines.append("  （无后续步骤）")
        ...
        return {
            K("approved"): action == "continue",
            K("review_feedback"): response.get("feedback", ""),
            K("review_action"): action,
    ...
    def execute_step(state) -> Dict:
        steps = state[K("workflow_steps")]
        current_idx = state[K("current_step_idx")]
    ...
    def step_executor_node(state):
        ...
        if status == "error":
            ...
            step_num = state[K("workflow_steps")][state[K("current_step_idx")]]["step"]
            interrupt_request = {
            ...
            if action == "retry":
                ...
                cleaned_outputs = dict(state[K("step_outputs")])
                cleaned_outputs.pop(step_key, None)
                return {
                    K("step_outputs"): cleaned_outputs,
                    K("current_step_idx"): state[K("current_step_idx")],
                    K("error"): None,
                ...
                return {
                    K("review_action"): "abort",
                    K("review_feedback"): f"执行错误后用户终止: {error_msg}",
                    K("final_output"): {
                        "skill": state[K("selected_skill")],
                        "status": "aborted",
                        "reason": f"执行错误后用户终止: {error_msg}",
                        "step_outputs": state[K("step_outputs")]
                    }
                }

        if status == "step_completed":
            return {
                K("current_step_idx"): result[K("current_step_idx")],
                K("step_outputs"): result.get(K("step_outputs"), {})
            }
    ...
    def single_executor_node(state):
        ...
        for idx, step in enumerate(workflow):
            ...
            print(f"[DEBUG] single_executor({domain}): 步骤{idx + 1} tool={tool_name} script={script_path}")
            print(f"[DEBUG] single_executor({domain}): 命令行参数: {sa_str}")

            if tool_name == "run_python":
                result = run_python.invoke({"script_path": params["script_path"], "script_args": sa_str})
                is_success = not result.startswith(("错误", "执行失败"))
                results.append({"tool": "run_python", "script": params["script_path"], "description": step.get("description", ""), "result": result, "success": is_success})
                if not is_success:
                    has_error = True
                    break
            elif tool_name == "run_r":
                ...
                is_success = not result.startswith(("错误", "执行失败"))
                results.append({"tool": "run_r", "script": params["script_path"], "description": step.get("description", ""), "result": result, "success": is_success})
                if not is_success:
                    has_error = True
                    break
    ...
    def continue_node(state):
        return {
            K("current_step_idx"): state[K("current_step_idx")] + 1,
            K("review_action"): None,
            K("review_feedback"): None,
            K("approved"): None
        }
    ...
    def abort_node(state):
        reason = state.get(K("review_feedback")) or "用户终止"
        existing = state.get(K("final_output"))
        ...
        return {
            K("final_output"): {
                ...
                "status": "aborted",
                "reason": reason,
                "completed_steps": state[K("step_outputs")]
            }
        }
    ...
    def report_results_node(state):
        return {K("execution_mode"): "chat"}
    ...
    def chat_router(state):
        ...
        if hasattr(last_message, 'tool_calls') and last_message.tool_calls:
            ...
            for tc in last_message.tool_calls:
                if tc["name"] == "launch_skill":
                    print(f"[DEBUG] chat_router({domain}): 检测到 launch_skill 调用, 参数: {tc['args']}")
                    return "tools_then_launch"
            print(f"[DEBUG] chat_router({domain}): 普通工具调用: {tool_names}")
            return "tools"
        ...
        if last_message.content and any(
            ...
            if not state.get(K("final_output")):
                print(f"[DEBUG] chat_router({domain}): ⚠️ LLM 编造执行结果，插入纠正消息重新调用")
                return "retry_with_correction"

        print(f"[DEBUG] chat_router({domain}): LLM 直接回复, 内容: {content_preview}")
        return "respond"
        ...
        if launch_tc:
            ...
            for msg in reversed(state["messages"]):
                if isinstance(msg, ToolMessage) and msg.tool_call_id == tc_id:
                    ...
                    print(f"[DEBUG] after_tools_router({domain}): launch_skill 返回: {content_preview}")
                    if "launch_skill" in msg.content:
                        print(f"[DEBUG] after_tools_router({domain}) → prepare_launch")
                        return "prepare_launch"
                    print(f"[DEBUG] after_tools_router({domain}) → chat (launch_skill 返回错误)")
                    return "chat"

        print(f"[DEBUG] after_tools_router({domain}) → chat (无 launch_skill)")
        return "chat"

    def mode_router(state):
        mode = state.get(K("execution_mode"), "chat")
        if mode == "workflow":
            return "workflow"
    ...
    def subgraph_entry_router(state):
        mode = state.get(K("execution_mode"), "chat")
        if mode == "idle":
            return "idle"
        if mode == "workflow" and state.get(K("workflow_steps")):
            return "executor"
        elif mode == "single_skill" and state.get(K("skill_config")):
    ...
    def workflow_router(state):
        if state[K("current_step_idx")] >= len(state[K("workflow_steps")]):
            return "workflow_complete"

        action = state.get(K("review_action"))
        if action == "abort":
...
def _build_parent_system_prompt() -> str:
    ...
    return f"""你是生态环境调查项目的协调助手，支持陆生动物与陆生植物两类数据分析。数据分析任务由规划器（planner）负责制定工作流，并并行交给对应领域的数据分析子图执行（动物→animal_date_analysis_agent，植物→plant_date_analysis_agent），你负责：
...
- 任务只涉及动物数据时，tasks 只含一个 domain="animal" 的任务；只涉及植物数据时，tasks 只含一个 domain="plant" 的任务；同时涉及动物和植物时，tasks 含两个任务，分别对应两个 domain。
- 每个任务形如：{"domain": "animal"|"plant", "mode": "workflow"|"single_skill", "skill_id": "<id>", "task_name": "...", "parameters": {...}, "workflow_steps": [...]}
...
- 用户说"全工作流/完整流程"时，复用目录中 type=workflow 的技能（动物用 full_animal_workflow，植物用 full_plant_workflow）的 workflow_steps。
- 无法确定工作流且缺少必要参数时，用 action=chat 交给协调助手向用户澄清；一般性问题（概念解释、闲聊、查看文件等）也用 action=chat。

输出 JSON：
{"action": "workflow"|"single_skill", "tasks": [task1, task2, ...]}
或
{"action": "chat"}
...
def _normalize_plan(plan: Dict) -> Dict:
    """把旧版单任务计划规整为统一的 tasks 列表结构。"""
    if not plan:
        return plan
    if plan.get("action") not in ("workflow", "single_skill"):
        return plan
...
def _prepare_domain_dispatch(state: ParentState, domain: str, task: Dict) -> Dict:
    """为指定 domain 构建命名空间执行状态。"""
    p = f"{domain}_"
    registry = _registry_for_domain(domain)
    mode = task.get("mode") or ("single_skill" if task.get("action") == "single_skill" else "workflow")
    params = task.get("parameters") or {}
    skill_id = task.get("skill_id") or task.get("skill_name")
    task_name = task.get("task_name")

    update = {
        p + "skill_params": params,
    ...
    if mode == "single_skill":
        ...
        if cfg is None:
            update[p + "execution_mode"] = "idle"
            update[p + "error"] = err
            return update
        update.update({
            p + "execution_mode": "single_skill",
            p + "selected_skill": cfg.get("name"),
            p + "skill_config": cfg,
            p + "workflow_steps": [],
        })
    ...
    steps = _normalize_workflow_steps(task.get("workflow_steps") or [])
    if not steps:
    ...
    if not steps:
        update[p + "execution_mode"] = "idle"
        update[p + "error"] = "缺少 workflow_steps，无法执行。"
        return update

    task_name = task_name or skill_id or ("陆生植物数据分析" if domain == "plant" else "陆生动物数据分析")
    skill_config = {
        "name": task_name,
        "id": skill_id or "planned_workflow",
        "description": task.get("reason") or "由父图规划生成的工作流",
        "type": "workflow",
        "parameters": {
    ...
    update.update({
        p + "execution_mode": "workflow",
        p + "selected_skill": task_name,
        p + "skill_config": skill_config,
...
请输出 JSON："""
    ...
    plan = _normalize_plan(_parse_json_safely(content))
    print(f"[DEBUG] parent_planner: 计划 action={plan.get('action')}, tasks={len(plan.get('tasks', []))}")
...
def parent_dispatch_node(state: ParentState) -> Dict:
    """并行分发节点：为两个子图分别准备命名空间状态。

    两个子图通过静态 fan-out 并行执行；未要求的 domain 被置为 idle（子图入口直接 END）。
    """
    plan = state.get("parent_plan") or {}
    tasks = plan.get("tasks") or []
    if not tasks and plan.get("skill_id"):
        tasks = [_normalize_plan(plan)["tasks"][0]]

    update = {"subgraph_delegated": True}
    requested = set()

    for task in tasks:
        domain = task.get("domain")
        if domain not in ("animal", "plant"):
            skill_id = task.get("skill_id") or task.get("skill_name") or ""
            domain = "plant" if str(skill_id).startswith("plant_") else "animal"
        requested.add(domain)
        update.update(_prepare_domain_dispatch(state, domain, task))

    # 未要求的 domain 置 idle，子图入口直接结束
    for domain in ("animal", "plant"):
        if domain not in requested:
            p = f"{domain}_"
            update[p + "execution_mode"] = "idle"
            update[p + "final_output"] = None
...
def _format_domain_result(domain: str, final_output: Optional[Dict]) -> Optional[str]:
    if not final_output:
        return None
...
def parent_plan_node(state: ParentState) -> Dict:
    """汇合节点：两个子图并行执行完成后，汇总各自结果并反馈给 parent_chat。"""
    msgs = []
    for domain in ("animal", "plant"):
        final_output = state.get(f"{domain}_final_output")
        msg = _format_domain_result(domain, final_output)
        if msg:
...
# 并行 fan-out：parent_dispatch 同时触发两个子图
parent_builder.add_edge("parent_dispatch", "animal_subgraph")
parent_builder.add_edge("parent_dispatch", "plant_subgraph")

# 并行 fan-in：两个子图都完成后才进入 parent_plan
...
def _print_last_ai_message(agent, config):
    ...
    for msg in reversed(messages):
        if isinstance(msg, AIMessage) and msg.content:
            ...
            return
    for domain in ("animal", "plant"):
        final = state.values.get(f"{domain}_final_output")
        if final:
            print(f"\n🤖 {domain} 执行结果: {json.dumps(final, ensure_ascii=False, indent=2, default=str)}")
            return
...
def run_interactive():
    ...
    print("=" * 60)
    print("生态环境调查数据分析助手（陆生动物 + 陆生植物，可并行执行）")
